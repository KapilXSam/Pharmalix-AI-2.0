...
import type { Consultation, DoctorView, ConsultStatus, DoctorDashboardStats, UpcomingAppointment, RecentActivity } from '../../types';
import { supabase } from '../../lib/supabaseClient';
import LoadingState from '../LoadingState';
import ErrorState from '../ErrorState';
import { Users, CalendarDays, Clock, UserPlus, ClipboardCheck, X, Check } from 'lucide-react';
...
const ConsultationRequestCard: React.FC<{ 
    consultation: Consultation;
    onUpdateStatus: (id: number, status: ConsultStatus) => void;
}> = ({ consultation, onUpdateStatus }) => (
    <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-lg flex items-center justify-between animate-fadeIn">
        <div className="flex items-center space-x-3 overflow-hidden">
            <LazyImage src={consultation.patient_avatar || `https://i.pravatar.cc/40?u=${consultation.patient_id}`} alt="Patient" className="w-10 h-10 rounded-full flex-shrink-0" />
        ...
        <div className="flex items-center space-x-2 flex-shrink-0 ml-2">
            <Button variant="ghost" size="sm" className="text-[hsl(var(--destructive))] hover:bg-[hsl(var(--destructive))]/10" onClick={() => onUpdateStatus(consultation.id, 'Cancelled')}>
                <X className="h-4 w-4" />
            </Button>
            <Button size="sm" className="bg-green-500 hover:bg-green-600" onClick={() => onUpdateStatus(consultation.id, 'Confirmed')}>
        ...
        {appointment ? (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center space-x-4">
                    <LazyImage src={appointment.patient_avatar || `https://i.pravatar.cc/48?u=${appointment.patient_name}`} alt="Patient" className="w-12 h-12 rounded-full ring-2 ring-white/50" />
                    <div>
...
const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ setCurrentView, onManageConsultation }) => {
    ...
    const fetchDoctorData = useCallback(async () => {
        ...
        try {
            ...
            const [statsResult, appointmentsResult, activityResult, consultationsResult] = await Promise.all([
                ...
                supabase.from('appointments').select('id, start_at, status, profiles:patient_id(full_name, avatar_url)').eq('doctor_id', user.id).eq('status', 'Confirmed').gt('start_at', new Date().toISOString()).order('start_at', { ascending: true }).limit(1),
                supabase.from('v_doctor_recent_activity').select('*').eq('doctor_id', user.id).order('event_timestamp', { ascending: false }).limit(5),
                supabase.from('consultations').select('*, profiles:patient_id (full_name, avatar_url)').eq('doctor_id', user.id).eq('status', 'Pending')
            ]);
            ...
            setPendingConsultations(enrichedData);
    ...
    const handleUpdateStatus = async (consultationId: number, status: ConsultStatus) => {
        ...
        <div className="space-y-8 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 items-start">
                <div className="lg:col-span-3 space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-md">
                        <h3 className="font-bold text-lg mb-3 text-slate-800 dark:text-white px-2">Consultation Requests ({pendingConsultations.length})</h3>
                        <div className="space-y-3">
                            {pendingConsultations.length > 0 ? (
                                pendingConsultations.map(c => <ConsultationRequestCard key={c.id} consultation={c} onUpdateStatus={handleUpdateStatus} />)
                            ) : (
                                <p className="text-slate-500 dark:text-slate-400 text-center py-4">No pending requests.</p>
                            )}
                        </div>
                    </div>
                </div>
