...
import CTIcon from './icons/CTIcon';
...
const AnalysisSection: React.FC<{ title: string; items: string[] | string }> = ({ title, items }) => {
    ...
    return (
         <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg">
            ...
            {Array.isArray(items) ? (
                ...
                <p className="text-sm text-slate-700 dark:text-slate-300">{parseMarkdown(items)}</p>
            )}
        </div>
    );
};
...
const CTAnalyzer: React.FC = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            // For DICOM files (.dcm), which may not have a browser-recognized MIME type,
            // we suggest a common image format to the API.
            const mimeType = imageFile.name.toLowerCase().endsWith('.dcm')
              ? 'image/jpeg' 
              : imageFile.type;

            const analysis = await analyzeCTScan({
            ...
            console.error('Error analyzing CT Scan:', err);
        ...
        switch (viewState) {
                ...
                return (
                    <>
                        ...
                        <ImageUploader
                            ...
                            icon={<CTIcon className="h-16 w-16 mx-auto text-sky-500 mb-4" />}
                            title="AI CT Scan Analyzer"
                            description="Upload your CT scan image for an in-depth, AI-powered clinical report."
                            accept="image/*, .dcm"
                        />
                    </>
                );
            ...
            case 'error':
                return (
                    <div className="animate-fadeIn max-w-3xl mx-auto space-y-6">
                        <LazyImage src={imageDataUrl!} alt="Uploaded CT Scan" className="rounded-xl shadow-lg w-full max-h-[50vh] bg-black" />
                         <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg space-y-4">
                             {viewState === 'loading' ? (
                                ...
                                <p className="text-slate-600 dark:text-slate-400">Click the button below to start the AI-powered analysis of the uploaded CT scan.</p>
                                <Button onClick={handleAnalyze} className="w-full">Analyze CT Scan</Button>
                                </>
                             )}
                        </div>
            ...
            case 'result':
                if (!result) return null;
                 const hasCriticalFinding = JSON.stringify(result.impression).includes("Immediate medical attention is required");
                return (
                    <>
                    {isChatting && <FollowUpChat reportTitle="CT Scan Report" reportData={result} onClose={() => setIsChatting(false)} />}
                     <div className="max-w-4xl mx-auto space-y-6">
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-lg space-y-6">
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">CT Scan Analysis Report</h2>
                             ...
                             {hasCriticalFinding && (
                                <div className="p-4 bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200 rounded-lg border-l-4 border-red-500">
                                    ...
                                    <p className="mt-1 text-sm">The Clinical Impression contains findings that may require immediate medical attention. Please consult a doctor urgently.</p>
                                </div>
                            )}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <AnalysisSection title="Modality" items={result.modality} />
                                <AnalysisSection title="Findings" items={result.findings[language]} />
                                <AnalysisSection title="Clinical Impression" items={result.impression[language]} />
                                <AnalysisSection title="Technical Notes" items={result.technical_notes[language]} />
                            </div>
                        </div>
                         <div className="flex flex-col sm:flex-row gap-4">
                            <Button onClick={handleReset} variant="outline" className="w-full">Analyze Another Scan</Button>
