...
import { LayoutDashboard, Users, AreaChart, Stethoscope, ArrowLeft, ArrowRight } from 'lucide-react';
...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ currentView, setCurrentView, isCollapsed, setIsCollapsed }) => {
  const navItems = [
    { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard className="h-5 w-5" /> },
    { id: 'users', label: 'User Management', icon: <Users className="h-5 w-5" /> },
    { id: 'analytics', label: 'Analytics', icon: <AreaChart className="h-5 w-5" /> },
  ];
  ...
  return (
    <aside className={`relative sidebar-container flex-shrink-0 text-white flex flex-col p-4 shadow-2xl transition-all duration-300 ease-in-out m-4 rounded-2xl ${isCollapsed ? 'w-24' : 'w-72'}`}>
      <div className={`flex items-center mb-10 pt-2 ${isCollapsed ? 'justify-center px-0' : 'px-4'}`}>
        ...
        <h1 className={`text-xl font-bold tracking-wider whitespace-nowrap text-white ${isCollapsed ? 'hidden' : 'block'}`}>Admin Panel</h1>
      </div>
