...
interface AnalysisResult {
    disclaimer: string;
    body_part_and_sequences: string;
...
const MRIAnalyzer: React.FC = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const analysis = await analyzeMRIScan({
    ...
    const renderContent = () => {
        switch (viewState) {
                ...
                return (
                    <>
                        ...
                        <ImageUploader
                            ...
                            icon={<MRIIcon className="h-16 w-16 mx-auto text-sky-500 mb-4" />}
                            title="AI MRI Scan Analyzer"
                            description="Upload your MRI scan image for an in-depth, AI-powered clinical report."
                            accept="image/*, .dcm"
                        />
                    </>
                );
            ...
            case 'error':
                 return (
                    <div className="animate-fadeIn max-w-3xl mx-auto space-y-6">
                        <LazyImage src={imageDataUrl!} alt="Uploaded MRI Scan" className="rounded-xl shadow-lg w-full max-h-[50vh] bg-black" />
                         <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg space-y-4">
                             {viewState === 'loading' ? (
                                ...
                                <p className="text-slate-600 dark:text-slate-400">Click the button below to start the AI-powered analysis of the uploaded MRI scan.</p>
                                <Button onClick={handleAnalyze} className="w-full">Analyze MRI Scan</Button>
                                </>
                             )}
                        </div>
            ...
            case 'result':
                ...
                const hasCriticalFinding = JSON.stringify(result.impression).includes("Immediate medical evaluation is necessary");
                return (
                    <>
                    {isChatting && <FollowUpChat reportTitle="MRI Scan Report" reportData={result} onClose={() => setIsChatting(false)} />}
                    <div className="max-w-4xl mx-auto space-y-6">
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-lg space-y-6">
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">MRI Scan Analysis Report</h2>
                             {hasCriticalFinding && (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <AnalysisSection title="Body Part & Sequences" items={result.body_part_and_sequences} />
