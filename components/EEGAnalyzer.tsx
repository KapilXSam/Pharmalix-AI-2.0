...
import EEGIcon from './icons/EEGIcon';
import ErrorState from './ErrorState';
import ImageUploader from './ImageUploader';
import { analyzeEEG } from '../services/geminiService';
...
interface AnalysisResult {
    disclaimer: string;
    background_activity: BilingualText;
    abnormal_patterns: BilingualStringArray;
    epileptiform_discharges: BilingualStringArray;
...
const EEGAnalyzer: React.FC = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            const analysis = await analyzeEEG({
    ...
    const renderContent = () => {
        switch (viewState) {
                ...
                return (
                    <>
                        ...
                        <ImageUploader
                            ...
                            icon={<EEGIcon className="h-16 w-16 mx-auto text-sky-500 mb-4" />}
                            title="AI EEG Analyzer"
                            description="Upload your EEG report image for a detailed, AI-powered clinical interpretation."
            ...
            case 'error':
                 return (
                    <div className="animate-fadeIn max-w-3xl mx-auto space-y-6">
                        <LazyImage src={imageDataUrl!} alt="Uploaded EEG" className="rounded-xl shadow-lg w-full max-h-[50vh] bg-white" />
                         <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-lg space-y-4">
                             {viewState === 'loading' ? (
                                ...
                                <p className="text-slate-600 dark:text-slate-400">Click the button below to start the AI-powered analysis of the uploaded EEG.</p>
                                <Button onClick={handleAnalyze} className="w-full">Analyze EEG</Button>
                                </>
                             )}
                        </div>
            ...
            case 'result':
                ...
                const hasCriticalFinding = JSON.stringify(result.impression).includes("urgent neurological evaluation");

                return (
                    <>
                    {isChatting && <FollowUpChat reportTitle="EEG Report" reportData={result} onClose={() => setIsChatting(false)} />}
                    <div className="max-w-4xl mx-auto space-y-6">
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-lg space-y-6">
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white">EEG Interpretation Report</h2>
                            {hasCriticalFinding && (
                                <div className="p-4 bg-red-100 dark:bg-red-900/50 text-red-800 dark:text-red-200 rounded-lg border-l-4 border-red-500">
                                    <h3 className="font-bold">! Urgent Evaluation Recommended</h3>
                                    <p className="mt-1 text-sm">The Clinical Impression contains findings that may require urgent neurological evaluation. Please consult a doctor promptly.</p>
                                </div>
                            )}
                            ...
                            <ResultSection title="Background Activity">
                                <p>{parseMarkdown(result.background_activity[language])}</p>
                            </ResultSection>

                            <ResultSection title="Abnormal Patterns">
                                {result.abnormal_patterns[language].length > 0 ? (
                                    <ul className="list-disc list-inside space-y-1">
                                        {result.abnormal_patterns[language].map((item, i) => <li key={i}>{parseMarkdown(item)}</li>)}
                                    </ul>
                                ) : <p>None noted.</p>}
                            </ResultSection>

                            <ResultSection title="Epileptiform Discharges">
                                {result.epileptiform_discharges[language].length > 0 ? (
                                    <ul className="list-disc list-inside space-y-1">
                                        {result.epileptiform_discharges[language].map((item, i) => <li key={i}>{parseMarkdown(item)}</li>)}
                                    </ul>
                                ) : <p>None noted.</p>}
                            </ResultSection>

                            <ResultSection title="Clinical Impression">
                             ...
                             {result.technical_notes && result.technical_notes[language].length > 0 && (
                                 <ResultSection title="Technical Notes">
                         ...
                         <div className="flex flex-col sm:flex-row gap-4">
                            <Button onClick={handleReset} variant="outline" className="w-full">Analyze Another EEG</Button>
